...
class EPGM(object):
    ...
    def _progress(self, item_name, n, step, arg, i):
        ...
        if (i + 1) % step == 0:
            print('{}> {} {} out of {} processed ({}%)'.format('-' * int((i / n) * 80), item_name, i + 1, n,
                                                               round((i + 1) / n * 100, 1)))
        return arg
    ...
    def _json_to_epgm(self, G_json, node_attributes, node_labels):
        ...
        G_epgm = {'graphs': [OrderedDict()], 'vertices': [OrderedDict(n) for n in G_json['nodes']],
                  'edges': [OrderedDict(e) for e in G_json['links']]}

        # update G_epgm['graphs']:
        G_epgm['graphs'][0].update(
            OrderedDict([('id', G_json['graph']['id']), ('data', {}), ('meta', {'label': G_json['graph']['name']})]))
        ...
        for ind, v in enumerate(G_epgm['vertices']):
            if node_attributes is not None:
                data = {str(k): str(v) for k, v in node_attributes.iloc[ind].items() if
                        v != 0}  # extract non-zero node attributes (sparse node attributes)
            else:
                data = {}
            v.update(OrderedDict(
                [('data', data), ('meta', {'label': str(node_labels[ind]), 'graphs': [G_json['graph']['id']]})]))

        # update G_epgm['edges']:
        edges_key_order = ['id', 'source', 'target', 'data', 'meta']
        for e in G_epgm['edges']:
            e.update(OrderedDict(
                [('id', uuid.uuid4().hex), ('data', {}), ('meta', {'label': '', 'graphs': [G_json['graph']['id']]})]))
        # reorder keys in e to edges_key_order:
        G_epgm['edges'] = [OrderedDict((k, e[k]) for k in edges_key_order) for e in G_epgm['edges']]
    ...
    def _reorder_keys(self, G):
        ...
        G['graphs'] = [OrderedDict((k, g[k]) for k in graphs_key_order) for g in G['graphs']]

        # Vertices:
        vertices_key_order = ['id', 'data', 'meta']
        # reorder keys in v to vertices_key_order:
        G['vertices'] = [OrderedDict((k, v[k]) for k in vertices_key_order) for v in G['vertices']]

        # Edges:
        edges_key_order = ['id', 'source', 'target', 'data', 'meta']
        # reorder keys in e to edges_key_order:
        G['edges'] = [OrderedDict((k, e[k]) for k in edges_key_order) for e in G['edges']]
    ...
    def load(self, path):
        ...
        for k in G_epgm.keys():
            ...
            #     enc = chardet.detect(fb.read(1000*len(line)))['encoding']   # detect the encoding from up to 1000 first lines in the file
            ...
            if isinstance(G_epgm[k], list):
                ...
                raise Exception('type(G[', k, ']): unexpected type', type(G_epgm[k]), ', stopping.')
    ...
    def node_types(self, graph_id):
        ...
        node_types = [v['meta']['label'] for v in self.G['vertices'] if graph_id in v['meta']['graphs']]
    ...
    def node_attributes(self, graph_id, node_type):
        ...
        x_ind = []
        nodes = [v for v in self.G['vertices'] if
                 (graph_id in v['meta']['graphs']) and (node_type in v['meta']['label'])]
        for v in nodes:
    ...
    def append(self, G_add):
        ...
        if any([G_json_add['graph']['id'] in g['id'] for g in self.G['graphs']]):
            raise Exception("Graph with id {} already exists".format(G_json_add['graph']['id']))
        ...
        for v_id in common_vertices:
            i = [v['id'] == v_id for v in self.G['vertices']].index(True)  # find the vertex index in self.G
            self.G['vertices'][i]['meta']['graphs'].append(G_epgm_add['graphs'][0]['id'])

        # append new vertices to self.G['vertices']:
        for v_id in new_vertices:
            i = [v['id'] == v_id for v in G_epgm_add['vertices']].index(
                True)  # find the index of v_id in G_epgm_add['vertices']
            self.G['vertices'].append(G_epgm_add['vertices'][i])  # add the vertex v_id to self.G['vertices']
        ...
        for e_id in common_edges:
            i = [(e['source'], e['target']) == e_id for e in self.G['edges']].index(
                True)  # find the edge index in self.G
        ...
        for e_id in new_edges:
            i = [(e['source'], e['target']) == e_id for e in G_epgm_add['edges']].index(
                True)  # find the index of e_id in G_epgm_add['edges']
            self.G['edges'].append(G_epgm_add['edges'][i])  # add the vertex e_id to self.G['edges']

    def to_nx_OLD(self, graph_id, directed=False, parallel_processing=True, n_jobs=multiprocessing.cpu_count(), progress=True, chunksize=100):
        """Convert the graph specified by its graph_id to networkx graph"""

        if graph_id in self.G_nx.keys():  # if self.G_nx[graph_id] already exists, just return it, otherwise evaluate it
            ...
            print('...[parallel_processing: {}, n_jobs: {}, progress_bar: {}]'.format(parallel_processing,n_jobs, progress))
            ...
            if parallel_processing:   # parallel execution
                ...
                if progress:
                    ...
                    pbar = ProgressBar(widgets=[SimpleProgress(format='%(value_s)s of %(max_value_s)s nodes processed (%(percentage)3d%%)')],
                                       maxval=n).start()
                    # _ = [pool.apply_async(partial(node_neighbours, edges=edges), args=(v,),
                    #                       callback=self.G_nx[graph_id].append) for v in nodes]   # it seems that appending results using callback works much slower than either pool.map_async or pool.map
                    # while len(self.G_nx[graph_id]) != n:
                    #     pbar.update(len(self.G_nx[graph_id]))
                    #     sleep(1)

                    graph = pool.imap(partial(node_neighbours, edges=edges), nodes, chunksize)   # lazy map
                    ...
                    self.G_nx[graph_id] = dict(pool.map(partial(node_neighbours, edges=edges), nodes))
                ...
                if progress:
                    ...
                    pbar = ProgressBar(widgets=[SimpleProgress(format='%(value_s)s of %(max_value_s)s nodes processed')],
                                       maxval=n).start()
                    self.G_nx[graph_id] = {self._progressbarUpdate(pbar, chunksize, v, i): [e[1] for e in edges if e[0] == v] for i, v
                             in enumerate(nodes)}  # this works ~2.5x faster (for cora dataset) than the above for loop
                    pbar.finish()

                else:
                    self.G_nx[graph_id] = {v: [e[1] for e in edges if e[0] == v] for v in nodes}  # this works ~2.5x faster (for cora dataset) than the above for loop
    ...
    def to_nx(self, graph_id, directed=False, *args):
        ...
        if False: #graph_id in self.G_nx.keys():  # if self.G_nx[graph_id] already exists, just return it, otherwise evaluate it
            return self.G_nx[graph_id]
    ...
    def adjacency(self, graph_id, directed=False):
        ...
        nodes = [v['id'] for v in self.G['vertices'] if graph_id in v['meta']['graphs']]
        adj = nx.adjacency_matrix(self.to_nx(graph_id, directed), nodelist=nodes)   # ensure the nodes in adj are ordered the same as in the epgm graph
    ...
    def edgelist(self, graph_id, directed=False):
        ...
        edgelist = [(e['source'], e['target']) for e in self.G['edges'] if graph_id in e['meta']['graphs']]   # works much faster, gets the edgelist directly from the edges.json part of epgm graph
        ...
        for k in self.G.keys():
            with open(os.path.join(self.path, str(k) + '.json'), 'w') as fp:
                if isinstance(self.G[k], list):
                    ...
                    print('type(G[', k, ']): unexpected type', type(self.G[k]), ', stopping.')
                    raise()
