...
def load_features(input_data):
    ...
    dt_collect = input_data[['eventid', 'nperps', 'success', 'suicide', 'nkill', 'nwound', 'gname']]
    dt_collect.fillna(0, inplace=True)
    dt_collect.nperps[dt_collect.nperps < 0] = 0

    summarize_by_gname = dt_collect.groupby('gname').agg({'eventid': 'count', 'nperps': 'sum', 
                                                          'nkill': 'sum', 'nwound': 'sum', 
                                                          'success': 'sum'}).reset_index()
    summarize_by_gname.columns = ['gname', 'n_attacks', "n_nperp", 'n_nkil', 'n_nwound', 'n_success']
    summarize_by_gname['success_ratio'] = summarize_by_gname['n_success']/summarize_by_gname['n_attacks']
    ...
    gname_attacktypes = dt_collect.groupby(["gname", "attacktype1_txt"])["attacktype1_txt"].count().to_frame()
    gname_attacktypes.columns = ['attacktype_count']
    gname_attacktypes.reset_index(inplace=True)
    gname_attacktypes_wide = gname_attacktypes.pivot(index='gname', columns='attacktype1_txt', values='attacktype_count')
    ...
    gname_targtypes = dt_collect.groupby(["gname", "targtype1_txt"])["targtype1_txt"].count().to_frame()
    gname_targtypes.columns = ['targtype_count']
    gname_targtypes.reset_index(inplace=True)
    gname_targtypes_wide = gname_targtypes.pivot(index='gname', columns='targtype1_txt', values='targtype_count')
    ...
    gnames_features = reduce(lambda  left,right: pd.merge(left,right,on=['gname'],
...
def load_network(input_data):
    ...
    dt_collect['country_decade'] = dt_collect['country_txt'] + '_' + dt_collect['decade'].map(str) +'s'
    dt_collect = dt_collect[dt_collect.gname != 'Unknown']

    # Create a country_decade edgelist
    gnames_country_decade = dt_collect.groupby(['gname','country_decade']).agg({'eventid': 'count'}).reset_index()
    gnames_country_decade_edgelist = pd.merge(gnames_country_decade, gnames_country_decade, on='country_decade', how='left')
    gnames_country_decade_edgelist.drop(['eventid_x', 'eventid_y'], axis=1, inplace=True)
    gnames_country_decade_edgelist.columns = ['source', 'country_decade', 'target']
    gnames_country_decade_edgelist = gnames_country_decade_edgelist[gnames_country_decade_edgelist.source != gnames_country_decade_edgelist.target]
    
    G_country_decade = nx.from_pandas_edgelist(gnames_country_decade_edgelist, source='source', target='target')
    ...
    G_related = nx.parse_adjlist(dt_collect.values, delimiter=', ') # attacks that are related
    df_related = nx.to_pandas_edgelist(G_related)
    df_related.replace(' ', '', regex=True, inplace=True)
    df_source_gname = pd.merge(df_related, gname_event_mapping, how='left', left_on='source', right_on='eventid')
    df_source_gname.rename(columns={'gname':'gname_source'}, inplace=True)
    df_target_gname = pd.merge(df_source_gname, gname_event_mapping, how='left', left_on='target', right_on='eventid')
    df_target_gname.rename(columns={'gname':'gname_target'}, inplace=True)
    
    # Filtering and cleaning
    gnames_relations_edgelist = df_target_gname[df_target_gname.gname_source != df_target_gname.gname_target]
    gnames_relations_edgelist = gnames_relations_edgelist[gnames_relations_edgelist.gname_source != 'Unknown']
    gnames_relations_edgelist = gnames_relations_edgelist[gnames_relations_edgelist.gname_target != 'Unknown']
    gnames_relations_edgelist = gnames_relations_edgelist[['gname_source', 'gname_target']]
    gnames_relations_edgelist.dropna(inplace=True)
    
    G_rel = nx.from_pandas_edgelist(gnames_relations_edgelist, source='gname_source', target='gname_target')
