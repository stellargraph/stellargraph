...
TRUE_TEXT = "yes"
# FIXME: this should point to stable, not latest
DOC_URL_BASE = "https://stellargraph.readthedocs.io/en/latest"
AUTOGENERATED_PROMPT = f"autogenerated by {__file__}, edit that file instead of this location"
DOCS_LINK_SEPARATOR = "\n<!-- DOCS LINKS -->\n"
...
def tables(action):
    ...
    for file_fmt in FILES:
        ...
        separate_compare_overwrite(file_name, separator, action=action, new_middle=new_table, label="table")
...
def demo_indexing(action):
    ...
    for directory in glob.iglob("demos/**/", recursive=True):
        ...
        if not os.path.exists(index):
            error(f"expected each demo README to match a docs 'index.txt' file, found `{readme}` without corresponding `{index}`")

        link = f"{DOC_URL_BASE}/{directory}"
        if directory != root_dir:
            # the root readme already has the table in it
            listing = "\n".join(demo_list(directory))
        else:
            listing = ""
...
{listing}"""
        ...
        separate_compare_overwrite(readme, DOCS_LINK_SEPARATOR, action=action, new_middle=new_contents, label="docs link")
...
def error(message, edit_fixit=False):
    formatted = (
        f"Error while generating information for documentation: {message}"
    )
    if edit_fixit:
