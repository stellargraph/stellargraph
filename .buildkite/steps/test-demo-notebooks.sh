...
case $(basename "$f") in
  'attacks_clustering_analysis.ipynb' | 'hateful-twitters-interpretability.ipynb' | 'hateful-twitters.ipynb' | 'stellargraph-attri2vec-DBLP.ipynb' | \
    ...
    'ensemble-link-prediction-example.ipynb' | 'movielens-recommender.ipynb' | 'stellargraph-metapath2vec.ipynb' | 'ensemble-node-classification-example.ipynb')
...
exitCode=0
papermill --execution-timeout=600 -p epochs 2 -p walk_length 5 --log-output "$f" "$f" || exitCode=$?
