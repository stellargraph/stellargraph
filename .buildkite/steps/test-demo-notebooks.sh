...
case $(basename "$f") in
  'attacks_clustering_analysis.ipynb' | 'hateful-twitters-interpretability.ipynb' | 'hateful-twitters.ipynb' | 'stellargraph-attri2vec-DBLP.ipynb' | \
    ...
    'movielens-recommender.ipynb' | 'stellargraph-metapath2vec.ipynb')
...
exitCode=0
papermill --execution-timeout=600 -p epochs 2 -p walk_length 5 -p batch_size 5 -p n_estimators 2 --log-output "$f" "$f" || exitCode=$?
