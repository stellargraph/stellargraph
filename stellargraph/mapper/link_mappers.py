...
class NodeSequence(Sequence):
    ...
    def _infer_head_node_types(self):
        ...
        if len(head_node_types) != 1:
            raise RuntimeError("All (src,dst) node types for inferred links must be of the same type!")
    ...
    def __getitem__(self, batch_num: int):
        ...
        head_edges = self.ids[start_idx:end_idx]
        ...
        # Get sampled features for head nodes
        batch_feats = self.generator.sample_features(head_edges, self._sampling_schema)
...
class GraphSAGELinkGenerator:
    ...
    def sample_features(self, head_links, sampling_schema):
        ...
        for hns in zip(*head_links):
            ...
            batch_feats.append([np.reshape(a, (head_size, -1, a.shape[1])) for a in feats])
        ...
        batch_feats = [
            feats for ab in zip(*batch_feats) for feats in ab
        ]
