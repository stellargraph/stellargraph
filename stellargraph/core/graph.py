...
class StellarGraph:
    ...
    above has two features ``x`` and ``y`` associated with each node::
        ...
        # As a RowFrame (no column names):
        feature_array = np.array([[-1, 0.4], [2, 0.1], [-3, 0.9], [4, 0]])
        nodes = RowFrame(feature_array, index=["a", "b", "c", "d"])
    ...
    Construction directly from a :class:`RowFrame` or NumPy array will have the least overhead, but
    ...
    :class:`RowFrame` or DataFrames in a dictionary. The dictionary keys are the names/identifiers
    ...
    type ``bar``, the construction might look like::
        ...
        foo_nodes = RowFrame(np.array([[-1]]), index=["a"])
        bar_nodes = RowFrame(
    ...
    case. The node IDs for a NumPy array can be specified via the :class:`RowFrame` type.)
    ...
    Args:
        nodes (Numpy array, RowFrame, DataFrame or dict of hashable to RowFrame or Pandas DataFrame, optional):
            Features for every node in the graph. The values are taken as numeric
            node features of type ``dtype``. If there is only one type of node, a NumPy array, :class:`RowFrame` or DataFrame can be
            ...
            ``RowFrame(index=node_ids)``, where ``node_ids`` is a list of the node
            IDs. If this is not passed, the nodes will be inferred from ``edges`` with no features
            for each node.

        edges (Numpy array, RowFrame, DataFrame or dict of hashable to RowFrame or Pandas DataFrame, optional):
