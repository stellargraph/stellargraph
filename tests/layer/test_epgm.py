...
class Test_EPGM_IO_Homogeneous(object):
    ...
    def test_node_attributes(self):
        ...
        if self.epgm_input:
            assert len(node_attributes) == 1433, \
                "There should be 1433 unique node attributes; found {}".format(len(node_attributes))
        else:
            assert len(node_attributes) == 1434, \
                "There should be 1434 unique node attributes; found {}".format(len(node_attributes))
...
class Test_EPGM_IO_Heterogeneous(object):
    ...
    def test_load_epgm(self):
        ...
        assert len(labels_none) == 62, "yelp contains 52 'user' nodes with undefined '{}' attribute; {} such nodes found instead".format(self.target_attribute, len(labels_none))
    ...
    def test_node_attributes(self):
        ...
        assert self.target_attribute in node_attributes
        assert len(node_attributes) == 19, "There should be 19 unique node attributes; found {}".format(len(node_attributes))
