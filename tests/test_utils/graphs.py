...
def example_graph_random(
    feature_size=4, n_edges=20, n_nodes=6, n_isolates=1, is_directed=False, rand_feats=False
):
    ...
    if feature_size is None:
        ...
        nodes = pd.DataFrame(np.random.random((len(node_ids), feature_size)), index=node_ids)
    else:
        # Example features
